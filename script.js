...
function fetchUserName() {
  fetch('get_user_name.php') // URL para o arquivo PHP que retorna o nome
    .then(response => response.json())
    .then(data => {
      const nomeUsuario = data.nome || 'Usuário Anônimo'; // Caso não tenha nome, coloca um valor default
      // Atualiza o conteúdo do modal com o nome do usuário
      modal.innerHTML = `
        <h2 style="margin-bottom:10px;">Meu Perfil</h2>
        <img src="sprites/parado1.png" 
             alt="Avatar" 
             style="width:100px;height:100px;border-radius:10px;border:2px solid #fff;margin-bottom:10px;">
        <p><strong>Nickname:</strong> ${nomeUsuario}</p>
    ...
    .catch(error => {
      ...
      modal.innerHTML = `
        <h2 style="margin-bottom:10px;">Meu Perfil</h2>
        <p>Erro ao carregar as informações do perfil.</p>
        <button id="closeModal" style="background:#e74c3c;color:white;border:none;border-radius:8px;padding:8px 14px;margin-top:10px;cursor:pointer;">Fechar</button>
      `;
    });
}

// Adiciona evento de clique no botão para abrir o modal
openBtn.addEventListener("click", () => {
  // Antes de abrir o modal, chama a função para carregar o nome
  fetchUserName();
...
overlay.addEventListener("click", (e) => {
  if (e.target === overlay || e.target.id === "closeModal") {
    overlay.style.display = "none"; // Fecha o modal ao clicar no fundo ou no botão "Fechar"
  }
});
